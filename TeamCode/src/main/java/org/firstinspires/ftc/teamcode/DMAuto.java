...
@Autonomous(name = "DMAuto", group = "Concept")
//  @Disabled
...
//        telemetry.clear();
        ...
        robot.strafeLeftForTime(.4, 1.0 );
        robot.setPowerOfAllMotorsToForTime(.3, 0.9 );
        if (opModeIsActive()) {
            robot.timer.reset();
            while (opModeIsActive()) {
                ...
                if ( objectCode.equalsIgnoreCase(LABEL_NO_ELEMENT)){
                    robot.setPowerOfAllMotorsToForTime( .45, 1.2 );
                    robot.turnRightForTime(-.3, 0.1 );
                } else {
                    robot.strafeRightForTime(.4, 1.0 );
                    robot.setPowerOfAllMotorsToForTime( .45, 1.2 );
                    robot.strafeRightForTime(-.4, 0.9 );
                    robot.turnRightForTime(-.3, 0.2 );
                }

                // Shooter
                while ( true ){
                    if(robot.timer.seconds() < 2.0 ){
                        robot.shooter.setPower( .70 );
                        sleep(1000);
                    }
                    robot.shooter.setPower( .70 );
                    robot.conveyor.setPower(-1);
                    if(robot.timer.seconds() > 15)
                        break;
                }


                if ( objectCode.equalsIgnoreCase(LABEL_NO_ELEMENT)) {
                    robot.setPowerOfAllMotorsToForTime(.3, 0.8);
                    robot.turnRightForTime(-.4, 0.9 );
//                    while (!isRed()) {
//                        robot.setPowerOfAllMotorsTo( .2 );
//                    }
                    robot.setPowerOfAllMotorsToForTime( .3,0.2 );
                    robot.timer.reset();
                    while (robot.timer.seconds() <= .6) {
                        robot.wobbleGoalArm.setPower( -.55 );
                    }
                    robot.timer.reset();
                    ...
                    robot.timer.reset();
                    while (robot.timer.seconds() <= 1.5) {
                        robot.wobbleGoalArm.setPower(.6);
                    }
                } else if ( objectCode.equalsIgnoreCase(LABEL_SECOND_ELEMENT)) {
                    robot.setPowerOfAllMotorsToForTime(.4, 1);
                    robot.strafeRightForTime( .3, .7 );
                    robot.timer.reset();
                    while (robot.timer.seconds() <= .6) {
                        robot.wobbleGoalArm.setPower( -.55 );
                    }
...
/*
                if (objectCode.equalsIgnoreCase(LABEL_NO_ELEMENT)) {
                    ...
                    robot.timer.reset();
                    while (robot.timer.seconds() <= .6) {
                        robot.wobbleGoalArm.setPower( -.45 );
                    }
                    robot.timer.reset();
                    while (robot.timer.seconds() <= .4) {
                        robot.wobbleGoalArm.setPower(-.2);
                    ...
                    robot.strafeLeftForTime(.4, 3);
                    boolean go = true;
                    while (go){
                        if(robot.timer.seconds() < 1){
                            robot.shooter.setPower(.85);
                            sleep(1000);
                        }
                        robot.shooter.setPower(.85);
                        robot.conveyor.setPower(.85);
                        if(robot.timer.seconds() > 14){
                            go = false;
                        }
...
*/
                ...
                /*sleep(1000);
                robot.strafeLeftForTime(.45, 1.5);
                telemetry.clear();
                sleep(1000);
                robot.timer.reset();
                while (robot.timer.seconds() <=1){robot.shooter.setPower(1);}
                robot.timer.reset();
                while (keepGoing) {
                    ...
                    robot.conveyor.setPower(1);
                    while (robot.timer.seconds() >= 2.15 && robot.timer.seconds() <=4.5){
                        robot.shooter.setPower(0);
                    }
                    while (robot.timer.seconds() >= 6.3 && robot.timer.seconds() <= 12.1);{
                        robot.shooter.setPower(0);
                    }
                    keepGoing = false;
                }*/
